...
parTruth = np.concatenate(([zCircCenter(RcBest)], zCirc(RKnot, RcBest)))
...
def lnL(par):
   ...
   zKnot = par[1:] # height of the mirror at the spline nodes [mm]
   ...
   zSpline = UnivariateSpline(x, z, k=2, s=0)
   ...
   for iRMeas in range(nRMeas):
      result = zOffset + dZIntMeas[iRMeas]
      result -= zIntercept(zOffset + dZIntMeas[iRMeas], # source position
                           RMeas[iRMeas],   # mirror zone
                           0.,  # parameter not used here
                           z0=zSpline(RMeas[iRMeas]),   # mirror height
                           dz0=dzSpline(R[iRMeas])) # mirror height derivative
      lnl -= 0.5 * (result)**2 / sdZIntMeas[iRMeas]**2
...
p0[:,1:] = zPara(RKnot, RcBest)[None,:]  * np.ones((nwalkers, ndim-1))
