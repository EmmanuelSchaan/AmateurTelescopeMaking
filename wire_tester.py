...
dZIntMeas = np.array([0.4 ,0.015, 0., -0.02, -0.4])   # [milli inches]
sdZIntMeas = 0.01 * np.ones_like(dZIntMeas)   # [milli inches]
# convert to mm
dZIntMeas *= 1.e-3 * inchToMm # [mm]
sdZIntMeas *= 1.e-3 * inchToMm # [mm]
...
def lnL(par):
   ...
   for iR in range(len(R)):
      ...
      result -= zIntercept(zOffset + dZIntMeas[iR], # source position
                           ...
                           z0=zSpline(R[iR]),   # mirror height
                           dz0=dzSpline(R[iR])) # mirror height derivative
