...
testRef = "20210123wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.15, 0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([13.6625, 13.6625, 12.805, 11.065, 9.51875, 8.35375])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.077, 0.07717675, 0.04153312, 0.04015595, 0.04532866, 0.10472434]) # [mm]
sdZWireMeas /= 2.
'''

'''
# 2021/01/31, wire test, (axis 1)
# source is fixed
testRef = "20210131wireax1"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.15, 0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([13.82375, 13.82375, 12.7175, 10.90625, 9.3875, 8.01625])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.1052007, 0.1052007, 0.0376663, 0.02986951, 0.04981215, 0.10230927]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/01/31, wire test, (axis 2)
# source is fixed
testRef = "20210131wireax2"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.15, 0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([14.78875, 14.78875, 13.30625, 11.635, 10.13375, 8.8975])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.12421629, 0.12421629, 0.04113621, 0.045, 0.01780976, 0.05273756]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/02/06, wire test, (axis 1)
# source is fixed
testRef = "20210206wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([13.265, 12.6625, 11.1775, 9.3125, 7.96625])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.06773109, 0.0553963, 0.0701338, 0.03031089, 0.06822527]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/02/06b, wire test, (axis 1)
# source is fixed
testRef = "20210206wireb"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([11.775, 11.32, 10.75, 8.55875, 6.8275])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.5, 0.5, 0.12668859, 0.03398069, 0.07862093]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/02/07, wire test, (axis 1)
# source is fixed
testRef = "20210207wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([15.1225, 14.4, 13.74, 11.68, 10.175])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.0475, 0.05, 0.13, 0.01, 0.005]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/02/07b, wire test, (axis 1)
# source is fixed
testRef = "20210207wireb"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([13.915, 12.8475, 12.20125, 10.5725, 9.11875])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.07762087, 0.05226136, 0.02965953, 0.09100137, 0.10690972]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/02/12, wire test, (axis 1)
# source is fixed
testRef = "20210212wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([13.99, 13.035, 12.3825, 10.795, 9.415])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.08624094, 0.07474958, 0.0349106, 0.01274755, 0.05937171]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/02/21, wire test, (axis 1)
# source is fixed
testRef = "20210221wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([12.615, 12.02, 11.385, 10.06875, 8.86375])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.14361407, 0.04301163, 0.03102418, 0.06580036, 0.09933372]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/07/29, wire test, (axis 1)
# source is fixed
testRef = "20210729wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([19.11375, 18.84375, 18.21, 17.85125, 17.03125])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.1812931, 0.05017158, 0.01695582, 0.03646488, 0.06730295]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/09/30, wire test, (axis 1)
# source is fixed
testRef = "20210930wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([19.54625, 17.7075, 15.38125, 13.17375, 11.935])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.04601291, 0.06259992, 0.04641861, 0.02630946, 0.09604686]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/10/14, wire test, (axis 1)
# source is fixed
testRef = "20211014wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([18.1275, 16.96125, 15.11, 12.8125, 11.5675])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
sdZWireMeas = np.array([0.08003905, 0.04188899, 0.0720243, 0.03473111, 0.06378675]) # [mm]
sdZWireMeas /= 2.
'''
'''
# 2021/10/18, wire test, (axis 1)
# source is fixed
testRef = "20211018wire"
# Mirror zones to be measured
RMeas = D/2. * np.array([0.316, 0.548, 0.707, 0.837, 0.949])   # [mm]
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([16.8175, 15.92875, 14.36, 12.06625, 10.59375])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
...
nRMeas = len(RMeas)
# Measured wire positions (median of 6 runs)
dZWireMeas = - np.array([16.1025, 15.66875, 14.5475, 12.61, 11.4075])  # [mm]
dZWireMeas -= dZWireMeas.mean()
# hack: divide by two because fixed source, to go back to moving source case
dZWireMeas /= 2.
# Uncertainties (standard deviation on the mean of 6 runs)
